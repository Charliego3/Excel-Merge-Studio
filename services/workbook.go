...
func (w *Workbook) RemoveSheet(main utility.Main) {
	if wb, ok := utility.State.Workbooks[main.Workbook]; ok {
		...
		wb.SheetNames = slices.DeleteFunc(wb.SheetNames, func(name string) bool { return name == main.Sheet })
		delete(wb.Sheets, main.Sheet)
		utility.NewMerger(wb.ID, main.Sheet).DeleteSheetMergeCells()
...
func (w *Workbook) RemoveWorkbook(id string) bool {
	if _, ok := utility.State.Workbooks[id]; ok {
		...
		utility.State.WorkbookIds = slices.DeleteFunc(utility.State.WorkbookIds, func(wkId string) bool { return wkId == id })
		utility.NewMerger(id, "").DeleteWorkbookMergeCells()
		utility.State.App.Event.Emit("workbooks:updated")
		return true
	}
	return false
